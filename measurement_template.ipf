...
mmacro initexp()
	...
	InitScanController(srv_push=0)
	sc_ColorMap = "VioletOrangeYellow" // change the colormap (default=Grays)
	sc_sleep(0.2)


	/////  setup connections to instruments /////
	killVISA()            // close whatever is open so you can try again
	openResourceManager() //  open VISA resource manager

	manualInitGPIB("srs1, srs2, dmm18", "1,2,18", "0") // initialize GPIB instruments
	sc_sleep(0.2)

	manualInitSerial("ips2", "2") // initialize serial instruments
	sc_sleep(0.2)
