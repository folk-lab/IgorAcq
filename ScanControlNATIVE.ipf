...
function getCmdHistory()
	...
	printf "%s is %.0f kB \r", expFile, totalBytes/1000.0
	...
	variable bytes = 0
	variable readValnnn, readValnn, readVal
	variable startPos = 0, endPos = 0
	do
		fBinRead /B=0 /F=1 expRef, readVal
//		fprintf histRef, "%d: %s, %s\r", bytes, num2str(readVal), num2char(readVal)
//		
//		if(readVal==-94 && readValnn==-128 && readValnnn==-30 && startPos==0)
//			// you found a bullet point!
//			startPos = bytes+1
//		endif
//		
//		if(startPos!=0 && readVal==33)
//			endPos = bytes+1
//			break
//		endif
//		
//		readValnnn = readValnn
//		readValnn = readVal
		bytes += 1
