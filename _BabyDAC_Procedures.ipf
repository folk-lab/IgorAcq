...
//function setvolts(channel, mV, [BabyDACNum])  
//	variable mV, channel, BabyDACNum
...
//	
//	if ( ParamIsDefault(BabyDACNum))
//		BabyDACNum=7  // This is the number that is written on the BabyDAC, set by the first five red jumpers inside on the DAC's board (least significant digit on left)
//	endif
//	variable babyNo=ceil((channel+1)/4+(BabyDACNum-1))  // If you want to use 2 BabyDACs at the same time, pick, for example, babydac 3 and 4. Set the BabyDACNum to 3, and use channels 0 to 7
//	
...
//	if (channel<12)
//	if(abs(mV)>5000.001) 
//		print "The output voltage cannot be greater than +- 5000mV"
//		mv = sign(mV)*5000
//	endif
//	endif
...
function setvolts(channel, mV, [BabyDACNum])  
	...
	wave dacvals=root:dacvals
	wave/t dacvalstr=dacvalstr
	NVAR out_voltage=out_voltage, dacrange
	variable voltage,setpoint,v1,v2,v3,v4,v5,parity, babychannel
	string binarystr,cmd
	...
	if (channel<12)
	if(abs(mV)>round(dacrange/2+0.001)) 
		print "The output voltage cannot be greater than +- %d mV", round(dacrange/2)
		mv = sign(mV)*round(dacrange/2)
	endif
	endif
	if(channel<0 || channel >15)
		print "The channel # should be between 0 to 15"
	...
	setpoint=round((mV+round(dacrange/2))/dacrange*1048575)
...
function/s GetDACStatus()
	...
	sprintf winfcomments "BabyDAC:\r\tCH0 = %.2f mV\r\tCH1 = %.2f mV\r\tCH2 = %.2f mV\r\tCH3 = %.2f mV\r\tCH4 = %.2f mV\r\tCH5 = %.2f mV\r\tCH6 = %.2f mV\r\tCH7 = %.2f mV\r", dacvals[0][1], dacvals[1][1], dacvals[2][1], dacvals[3][1], dacvals[4][1], dacvals[5][1], dacvals[6][1], dacvals[7][1]
	return winfcomments
end

////////////////////////////////////////////////////////////////

function initBabyDAC([range])
	variable range
	variable /g dacrange
	...
	dacrange = range
	string cmd
	variable/g out_voltage=0, set_channel=0
	string /g babydacport = "COM19"
...
//	DrawText 28,477,"Lockin Address  SRS"
	...
	ListBox list0,selRow= 7,editStyle= 2
...
//	SetVariable lockinaddress,pos={146,462},size={30,16},title=" "
